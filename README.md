...
│   │
│   ├── services/
│   │   ├── ai_service.py        # ONLY file that talks to Groq
...
**Decision:** Use Groq's `response_format={"type": "json_object"}` instead of prompt engineering + regex parsing.
...
**How JSON mode works:**
```python
# GOOD — Groq physically cannot return invalid JSON
response = client.chat.completions.create(
    model="llama-3.3-70b-versatile",
    messages=[...],
    response_format={"type": "json_object"},  # Enforced at API level
)
result = GoalResponse.model_validate(json.loads(response.choices[0].message.content))
```
...
**Second layer:** Even after Groq guarantees the JSON, we run Pydantic validation on our side. Two lines of defence.
...
2. **Groq's Python SDK** is mature and well documented
...
**Decision:** All Groq AI code is isolated in `services/ai_service.py`.

This is the most important architectural decision for long-term maintainability.

**If you swap Groq for OpenAI tomorrow:**
```bash
# Only edit ONE file
...
**For production:** Add a circuit breaker (e.g., `pybreaker` library) that stops calling Groq after N consecutive failures and returns a friendly degraded response immediately, instead of making users wait for timeouts.
